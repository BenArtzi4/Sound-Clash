...
- 2026-05-24: Manager Bonus picker no longer pushes the Continue / Next-round row down when it opens, so the host's next tap can't accidentally land on a team button. Also bumped the Display sound toggle to a 44px tap target with a clearer screen-reader label, and disabled the iOS double-tap-zoom delay on every button (not just the BUZZ button).
