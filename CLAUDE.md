...
Sound Clash is a real-time multiplayer music-trivia buzzer game. **Phase 3 (Postgres logic) is complete** — the schema, the five PL/pgSQL functions (`buzz_in`, `start_round`, `award_points`, `end_game`, `cleanup_expired_games`), RLS policies, and the pg_cron sweeper are all in `db/migrations/`. The buzz race test (10 concurrent calls → 1 winner, 100×) is the headline exit criterion, gated in CI by the `e2e.yml buzz_race_stress` job. **Phase 4 (FastAPI routers / middleware / services) and Phase 5 (frontend) are not yet started** — `backend/app/{db,middleware,routers,services}/` and `frontend/src/{components,pages,lib,hooks}/` remain empty. Phases are tracked in `docs/roadmap.md`; never assume a feature exists in code just because docs describe it.
