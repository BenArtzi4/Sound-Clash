...
- Team page should work on iOS Safari and Chrome Android. iPhone SE viewport is the smallest supported size. During play each player's own **place + score** is shown on their buzzer screen (rank among all teams, same tiebreak as the Display), so a team off the top-5 board still knows where it stands.
