...
- **End of game — the setlist.** Under the final results, the host console and every player's phone list the songs that played, in round order, with release year and who got each one ("Got it: Alpha" when one team took both the song and the artist, "Song: Alpha · Artist: Bravo" when they split, "Nobody got it"). The first five show, with a *Show all* toggle. Actions: **Play all on YouTube** (YouTube's anonymous playlist holds 50 videos, so a longer game gets one link per 50 songs), **Share list** on a phone / **Copy list** on a laptop (plain `Artist - Title` lines — the format playlist converters such as TuneMyMusic read, which is the supported route to a Spotify or Apple Music playlist), and **Save as file** (a self-contained page in the game's look: winner, leaderboard, the setlist with play links). Everything is built in the browser from the game snapshot; nothing is stored. The Display shows no setlist — nobody interacts with the TV. A direct Spotify integration is deliberately not built: Spotify's Web API caps a non-company app at 5 hand-allowlisted users (2026) and its developer policy forbids trivia games.
