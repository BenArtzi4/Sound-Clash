...
- **T-CascadeTest · Pin the Realtime cascade-delete UX.** `[S]` `expiration.spec.ts` + the reducer test already cover the general "gone" banner; what's missing is a test for the **specific `game_teams`-before-`active_games` ordering** that routes players to Home instead of the expiry screen (F-P1-2). One reducer/e2e case pins that distinction.
