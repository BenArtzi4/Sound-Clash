...
- **T-Admin · Split `AdminSongsPage.tsx` (639 lines).** `[M/low]` Extract `SongTable`, `SongEditForm`, `useAdminSongs`. Also cleanly fixes the page-index clamp (prior finding) and enables I-Admin. **→ [issue #249](https://github.com/BenArtzi4/Sound-Clash/issues/249)** (open, autonomous, low-value).
