# Execution Contract — how to run any task

The single process doc for executing the plan (it absorbed the old `execution-playbook.md` in the 2026-07 reorg; the per-phase files retired in the 2026-07-14 cleanup — [TASKS.md](TASKS.md) is now the one backlog). **To start a session, paste a short prompt like:**

> Continue the Sound Clash plan. Read `docs/planning/NEXT-SESSION.md` first, then follow `docs/planning/EXECUTION-CONTRACT.md` and pick from `docs/planning/TASKS.md`.

Everything below applies to **every** task. [TASKS.md](TASKS.md) supplies the tasks and their per-task notes/flags; the exit gate is §5.

---

## 0 · What you're working on
A **live production** party game at `https://www.soundclash.org` — never break a running game. Read `docs/architecture.md` for the architecture if needed. **Do NOT touch the uncommitted `tools/song-curation/` files** (maintainer's in-flight work). Stage by explicit path; never `git add .`.

## 1 · Read first (every session)
1. `docs/planning/README.md` (status snapshot) + `docs/planning/NEXT-SESSION.md` (current handoff)
2. this file
3. `docs/planning/TASKS.md` — the backlog (per-task design notes + flags) — and/or the GitHub issue being worked
4. `docs/planning/DECISIONS.md` — decisions are **resolved**; don't re-litigate
...
Then `git fetch && git status` — confirm clean state, note in-flight branches/PRs — and pick the next unblocked task.

## 2 · Session sizing & model policy
- **Session model: Opus 4.8** unless the task notes say otherwise (strong coder; conserves the Fable/Mythos budget).
- One session = one task cluster, or several small independent tasks. If a task balloons: land what's safely shippable, update `TASKS.md` with what remains, stop cleanly — the next session picks up from the plan, not from memory.
...
- Reach for a **Workflow** (multi-agent fan-out) only when the work is "N independent instances of the same shape" (bulk test generation, multi-doc sweeps) or "needs adversarial confidence" (gate audits, security re-verify, review/verify fan-out on a finished diff). Everything else: single agent. Workflow outputs still go through the normal branch → PR → gate path.
...
8. **Tick** the task box in `TASKS.md` and refresh `NEXT-SESSION.md`; commit that (same PR or a tiny docs PR).
9. If the session discovered a new bug/idea, record it rather than fixing it ad hoc — a GitHub issue for autonomous work, `TASKS.md` §C for maintainer-gated work (exception: P0 production breakage — fix immediately, then record).
...
- Anything that changes architecture (new service, auth model, state library, non-YouTube audio), game rules/scoring, or costs money — ask first with a concrete recommendation (the decision protocol that produced `DECISIONS.md`).
...
**Full-Game Exit Gate** — a feature / task cluster is **not done** until all pass (run after its last PR merges and deploys — Render + Pages auto-deploy from `main`):
...
If any gate item fails: log it (a GitHub issue, marked P0/P1), fix it, re-run the gate. Do not declare the task done until the gate is fully green.

## 6 · When the task is done
- Tick the box in `TASKS.md`; refresh `NEXT-SESSION.md` (+ `README.md` if the status snapshot changes).
- Report every PR that merged + before/after numbers where relevant (latency, bundle size, Realtime message count).
- If a big change surfaced that isn't already resolved in `DECISIONS.md`, **stop and ask** the maintainer.
