...
_Last updated: 2026-07-09 (T4.11 shipped). **All T4.x tasks done/de-scoped; the only thing left in Phase 4 is the exit gate.**_

## Short prompt to paste into the fresh session

> **Continue the Sound Clash plan. Read `docs/planning/NEXT-SESSION.md` first, then run the Phase 4 exit gate: follow `docs/planning/phases/EXECUTION-CONTRACT.md` §5 and `docs/planning/phases/phase-4-resilience.md`'s Exit gate. Read `.claude/rules/lessons-learned.md` before running anything. The exit gate is a verification pass (all suites green + a deliberate "adverse" game on prod hitting ≥3 failure paths), not new feature work — most of the underlying failure-mode code already shipped in T4.2–T4.11. The maintainer is button-averse: prefer zero-UI/auto fixes and confirm before adding any button.**
...
## What to do next — the Phase 4 exit gate

All T4.x tasks are shipped or de-scoped. The remaining work is the **exit gate** (EXECUTION-CONTRACT §5 + `phase-4-resilience.md`'s Exit gate) — a verification pass, not new feature code:

1. **Run all local suites green** (backend+db pytest, frontend vitest+coverage, tsc, eslint, ruff, mypy). No buzz-path/RPC edits remain unverified — T4.11 touched only `useGameChannel`'s snapshot exposure + the three pages' end-of-game branches, no RPC/buzz change.
2. **e2e against a local `supabase start` stack** — at minimum `full_game`, `multi_buzz_round`, `buzzer_race`, plus `expiration.spec.ts` (extended in T4.11 to assert the final board on all three clients).
3. **A deliberate "adverse" game on prod** hitting ≥3 failure paths: dead video → Next round; host tab backgrounded → auto-resume [T4.2 ✅]; socket drop → reconnect with no lost events [T4.3 ✅]; near-expiry → warning shows, extend works [T4.8 ✅ — exercise live]; wiped host localStorage → backup host link recovers [T4.10 ✅ — e2e-covered, exercise live]; **End/expire a game → final board stays up on all three screens [T4.11 ✅ — exercise live]**.
4. When green: mark Phase 4 ✅ in `README.md` + this file; recommended next phase is **6 → 7 → 5 → 8** (see `phases/README.md`).
