...
>    - **Only leftover = mig 044 (a fresh-session follow-up, not urgent):** once no old browser tabs remain (hours/days), `DROP FUNCTION award_attempt(text,uuid,integer,integer,integer,uuid);` to retire the now-dead integer overload (mirrors mig 023). The boolean overload is the sole active path; the integer one just sits unused until then. Label the mig-044 PR `run-stress` + `run-e2e`.
...
**One-glance state:** Phases 1–6 ✅ complete + live on prod. **T7.1** scoring authority is **✅ live on prod + verified** (PR #218 merged, **mig 043 applied** — migrations through **043**; only mig 044 to drop the dead integer overload remains, a later follow-up). The tractable autonomous cleanups are **done** (T5.7, F-P2-6, T6.3, all dependabot). **What's left:** flag/infra items — **T7.6** CI (bundle budget / RLS CI job / e2e gate), **F-P2-5** rate-limit `--proxy-headers`, **T5.6** Cloudflare, **T5.2** name-policy call, **T5.4** history retention, **T7.4** lockfile un-ignore; then **Phase 8** features (needs your direction + the existing vetoes). Any `db/migrations/**` PR needs `run-stress` + `run-e2e` labels. Maintainer is button-averse (confirm before any UI). Autonomous `gh pr merge` / prod writes work only when your prompt explicitly authorizes them.
...
1. **mig 044 (T7.1 tail — do in a fresh session, low urgency)** — once no old browser tabs remain (hours/days after #218), `DROP FUNCTION award_attempt(text,uuid,integer,integer,integer,uuid);` to retire the dead integer overload. New PR + migration 044, labelled `run-stress` + `run-e2e`, then apply to prod + verify. Until then the dual overload is harmless (boolean is the only active path).
