...
_Last updated: 2026-07-09 (T4.10 shipped — PR #196). Mid-Phase-4; **T4.11 (optional) is next, then the Phase 4 exit gate**._

## Short prompt to paste into the fresh session

> **Continue the Sound Clash plan. Read `docs/planning/NEXT-SESSION.md` first, then continue Phase 4 at T4.11 (optional — decide with the phase file whether to build or de-scope it) and the Phase 4 exit gate: follow `docs/planning/phases/EXECUTION-CONTRACT.md` and `docs/planning/phases/phase-4-resilience.md`. Read `.claude/rules/lessons-learned.md` before running anything. Work the tasks one at a time — they cluster on a few shared files; put the parallelism inside each task (fan-out review/verify on the finished diff), not across tasks. The maintainer is button-averse: prefer zero-UI/auto fixes and confirm before adding any button.**
...
## What to do next — Phase 4 from T4.11 (recommended order)
...
1. **T4.11 · (optional) Final board survives delete `[M]`** — render the final scoreboard from last-known state on End/expiry instead of "This game no longer exists". Optional: decide build vs de-scope against real value before coding.
2. Then the **Phase 4 exit gate** — all suites + a deliberate **"adverse" game** hitting ≥3 failure paths (dead video → Next round; host tab backgrounded → auto-resume [done]; socket drop → reconnect with no lost events [T4.3 ✅]; near-expiry → warning shows, extend works [T4.8 ✅ — exercise it live]; wiped host localStorage → backup host link recovers [T4.10 ✅ — e2e-covered, exercise once live]).
