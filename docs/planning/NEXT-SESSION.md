...
**One-glance state:** Phases 1–6 ✅ complete + live on prod. **T7.1** scoring authority is **✅ fully wrapped + live on prod** — PR #218 (mig 043) **and** PR #220 (mig 044) both merged + applied + verified; **migrations through 044** on prod; the boolean `award_attempt` overload is now the **sole** signature (no scoring follow-ups remain). The tractable autonomous cleanups are **done** (T5.7, F-P2-6, T6.3, all dependabot). **What's left:** flag/infra items — **T7.6** CI (bundle budget / RLS CI job / e2e gate), **F-P2-5** rate-limit `--proxy-headers`, **T5.6** Cloudflare, **T5.2** name-policy call, **T5.4** history retention, **T7.4** lockfile un-ignore; then **Phase 8** features (needs your direction + the existing vetoes). Any `db/migrations/**` PR needs `run-stress` + `run-e2e` labels. Maintainer is button-averse (confirm before any UI). Autonomous `gh pr merge` / prod writes work only when your prompt explicitly authorizes them.
...
2. **Flag-first / infra (the tractable next tasks):** T7.4-Lockfile (verify the npm-10 runner bug is gone first, then un-ignore `frontend/package-lock.json`), T7.6 (CI job for RLS suite, bundle budget, e2e gate), F-P2-5 (`--proxy-headers`), T5.6 (Cloudflare), T5.4 (history retention window — decision).
