...
> 4. **T7.1 is the one big remaining item and needs care** — see "What to do next" + the refined plan in `phase-7-tech-debt.md`. It's a live-scoring `award_attempt` rewrite (**now mig 043**; 041/042 are taken) with a **delicate prod cutover**: use the **dual-overload** pattern (add the boolean signature alongside the integer one, don't drop it) so mig 043 is backward-compatible, **apply mig 043 to prod BEFORE merging** (merging first deploys a boolean-sending frontend against a prod with no boolean overload → live scoring breaks), then drop the integer overload later in mig 044. Best done in a fresh focused session.
...
**One-glance state:** Phases 1–6 ✅ complete + live on prod (migrations through **042**). The tractable autonomous cleanups are **done** (T5.7, F-P2-6, T6.3, all dependabot). **What's left:** **T7.1** scoring authority — the biggest item, a live-scoring `award_attempt` rewrite (**mig 043**, dual-overload, delicate prod cutover — see ⚠️ box + `phase-7-tech-debt.md`; best done fresh with the maintainer reachable during the cutover); then flag/infra items — **T7.6** CI (bundle budget / RLS CI job / e2e gate), **F-P2-5** rate-limit `--proxy-headers`, **T5.6** Cloudflare, **T5.2** name-policy call, **T5.4** history retention, **T7.4** lockfile un-ignore; then **Phase 8** features (needs your direction + the existing vetoes). Any `db/migrations/**` PR needs `run-stress` + `run-e2e` labels. Maintainer is button-averse (confirm before any UI). Autonomous `gh pr merge` / prod writes work only when your prompt explicitly authorizes them.
