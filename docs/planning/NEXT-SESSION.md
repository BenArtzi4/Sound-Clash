...
- **Redesign leftovers (small):** PR #340 (issue #337, console status-strip gap — merge-ready), issues #335 (smoke spec `www.` API base) and #336 (four `stale_buzz_lock_resynced` heals in the 13-tab games), and the three taste calls F-08 / F-09 / F-12 in the validation report. The report's "Owed to the maintainer" list has two optional device lines (two-phone game, TV glance) the maintainer may run or waive.
- **Nothing urgent / on fire. No open production or security holes.**

## What to do next (pick one)

1. ~~Finish the redesign~~ **done.** The final validation ran 2026-09-22 (GO), the border-token contrast fix shipped 2026-09-21 (PR #326) and the OG image + PWA icons in PR #327. What is left is one click and three decisions: merge PR #340, decide issues #335 / #336, and answer F-08 / F-09 / F-12 in `docs/planning/ui-redesign/validation/2026-09-22-final-validation.md` (a PR follows each answer).
