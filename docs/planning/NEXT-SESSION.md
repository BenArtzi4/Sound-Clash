...
>    - **mig 044 = PR #220 OPEN + handed off (2026-07-10).** `DROP FUNCTION award_attempt(text,uuid,integer,integer,integer,uuid);` retires the now-dead integer overload (mirrors mig 023); boolean signature becomes the sole one. Test + docs finalized to boolean-only; labelled `run-stress`+`run-e2e`; local db tests green (58 passed on a throwaway testcontainer). **Maintainer: merge + apply to prod** — buzz-path/prod-migration, so NOT self-merged. Apply only ≥1 game-TTL (4h) after the #218 deploy so no pre-#218 tab can still call the integer overload; then `SELECT count(*) FROM pg_proc WHERE proname='award_attempt'` → 1 and run `post_deploy.sh`. Rollback = re-apply mig 036.
...
**One-glance state:** Phases 1–6 ✅ complete + live on prod. **T7.1** scoring authority is **✅ live on prod + verified** (PR #218 merged, **mig 043 applied** — migrations through **043** on prod; **mig 044** (drop the dead integer overload) is **PR #220, open + handed off** for the maintainer to merge + apply). The tractable autonomous cleanups are **done** (T5.7, F-P2-6, T6.3, all dependabot). **What's left:** flag/infra items — **T7.6** CI (bundle budget / RLS CI job / e2e gate), **F-P2-5** rate-limit `--proxy-headers`, **T5.6** Cloudflare, **T5.2** name-policy call, **T5.4** history retention, **T7.4** lockfile un-ignore; then **Phase 8** features (needs your direction + the existing vetoes). Any `db/migrations/**` PR needs `run-stress` + `run-e2e` labels. Maintainer is button-averse (confirm before any UI). Autonomous `gh pr merge` / prod writes work only when your prompt explicitly authorizes them.
...
**T7.1 is live+verified on prod. Everything left needs the maintainer, a flag, or is off-limits.** (F-P2-6 shipped in #215, T6.3 in #216, T7.1 in #218 — all done + live.)

1. **mig 044 (T7.1 tail) — ✅ PR #220 OPEN + handed off.** Migration + test + docs done, labelled `run-stress` + `run-e2e`, local db tests green. **Maintainer: merge #220 + apply mig 044 to prod** (buzz-path/prod-migration → not self-merged) once ≥1 game-TTL (4h) has passed since the #218 deploy, then verify `count(*) pg_proc award_attempt` → 1 + `post_deploy.sh`. Until applied, the dual overload is harmless (boolean is the only active path).
