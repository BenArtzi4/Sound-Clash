...
2. **Flag-first / infra (the tractable next tasks):** T7.6 (CI job for RLS suite, bundle budget, e2e gate), F-P2-5 (`--proxy-headers`), T5.6 (Cloudflare), T5.4 (history retention window — decision). _(**T7.4-Lockfile + the whole CI npm-cache follow-up are ✅ DONE + merged, main green** — #224 tracked the lockfile, #225 cached `frontend.yml`, #226 cached `e2e.yml` + fixed its stale comment, #227 synced `e2e.yml` to Node 22. Nothing left here.)_
