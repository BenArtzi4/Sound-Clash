...
2. **Flag-first / infra (the tractable next tasks):** T7.6 (CI job for RLS suite, bundle budget, e2e gate), F-P2-5 (`--proxy-headers`), T5.6 (Cloudflare), T5.4 (history retention window — decision). _(T7.4-Lockfile ✅ shipped 2026-07-11, PR #224 — `frontend/package-lock.json` now tracked. Follow-up **PR #225** re-enabled the setup-node `cache: "npm"` in `frontend.yml` (maintainer-authorized CI change); the audit flagged `e2e.yml`'s now-stale untracked-lockfile comment + its uncached setup-node step as a further maintainer-decision follow-up.)_
