...
**Implementation steps (Design 1), as ONE PR stacked on #206:**
1. **mig 041**: `DROP FUNCTION award_attempt(text,uuid,integer,integer,integer,uuid);` then `CREATE OR REPLACE FUNCTION award_attempt(... booleans ...)` computing magnitudes server-side (keep the mig-036 single-UPDATE-RETURNING economy; the `*_points` columns now store DB-computed values); re-`GRANT`. Idempotent, applied twice locally. Label the PR **`run-stress` + `run-e2e`**.
...
7. **Prod rollout (maintainer):** the client must call the new signature, so apply mig 041 **atomically with / just before** the frontend deploy (mig-021 kept the old overload for exactly this transition — consider keeping the integer overload alive for one deploy if a zero-downtime cutover is wanted, then dropping it in a follow-up mig). Verify scoring values on prod after (title=10/artist=5/soundtrack=15/wrong=−3/bonus=+4).
