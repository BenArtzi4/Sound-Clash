...
| **Code field** | One `<input maxLength=6>`, `opacity: 0` and covering the box, over an `aria-hidden` `repeat(6, 1fr)` grid that draws the characters — `--font-display` 2 rem, uppercase, one glyph centred per column, dividers from `border-left`; placeholder `------` in `--text-dim`. (Built 2026-09-20, PR #317: `letter-spacing` cannot fill fixed cells in a proportional face — see `05-page-by-page.md`.) |
