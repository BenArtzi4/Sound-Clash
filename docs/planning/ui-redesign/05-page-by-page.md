...
- **Code field.** Still ONE `<input maxLength=6>` (tests and the QR deep-link prefill depend on it), styled as six cells in the display face at 2 rem. **As built** the characters are drawn by a six-column grid that the transparent input covers, not by `letter-spacing` over a `repeating-linear-gradient`: tracking cannot hold a proportional face in fixed cells, so a wide code drifted out of its boxes and the sixth character scrolled the whole run sideways (#317). Both entry screens share one component, `components/GameCodeField.tsx`, which also owns the editing the plan never specified (#321): the caret is drawn from the input's real selection rather than pinned to the next empty cell, a tap lands on the cell under the finger and selects that character, and once the code is full a keystroke overwrites the character at the caret — `maxLength` would otherwise swallow it, leaving a wrong character unfixable without deleting one first. Backspace, Delete, arrows, Home/End, select-all, drag-select, paste and autofill stay native. Auto-uppercase stays. When the code arrives from the URL the field is pre-filled and read-only-looking (muted), as today.
