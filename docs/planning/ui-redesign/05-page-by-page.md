...
- **Code field.** Still ONE `<input maxLength=6>` (tests and the QR deep-link prefill depend on it), but the characters you see are drawn by an `aria-hidden` six-column grid stacked under it; the input itself is `opacity: 0` and covers the whole box, so focus, keyboard, paste, autofill and hit-testing all stay on the real control. Display face at 2 rem, one glyph centred per `1fr` column, dividers from `border-left` (`:first-child` gets none), and `box-shadow: inset 0 -3px 0 var(--accent)` on the next empty cell under `:focus-within` as a caret stand-in. Auto-uppercase stays. When the code arrives from the URL the field is pre-filled and read-only-looking (muted), as today.
