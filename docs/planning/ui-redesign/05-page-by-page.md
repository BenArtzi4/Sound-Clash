...
- **Code field.** Still ONE `<input maxLength=6>` (tests and the QR deep-link prefill depend on it) but styled as six cells: monospace/tabular display face at 2 rem, `letter-spacing: 0.6em`, and a `repeating-linear-gradient` background that draws six equal boxes behind the characters. Auto-uppercase stays. When the code arrives from the URL the field is pre-filled and read-only-looking (muted), as today.
...
- **Layout** (≥ 769 px, fixed TV frame, no scroll): a 12-column grid. Left 8 columns = the board; right 4 columns = a sidebar with the round card (reveal rows + token chips), the countdown, and the QR panel pinned to the bottom. This removes the vertical stacking that pushes the QR card off a 720p screen and gives the reveal rows room for long Hebrew titles. Below 769 px the sidebar stacks under the board (existing mobile behaviour).
