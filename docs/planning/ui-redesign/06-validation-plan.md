...
4. **No new dependency without a line in the PR description explaining why the zero-dependency route was insufficient** (repo rule: `.claude/rules/dependencies.md`). The plan's default is zero new runtime dependencies (see [03-library-evaluation.md](03-library-evaluation.md)).
...
| **Bundle guard** — no animation library in player-reachable chunks | `index-*.js` and `TeamGameplayPage-*.js` contain none of `framer-motion`, `auto-animate`, `lenis`, `gsap` | A 10-line check after `npm run build` (vitest or a CI step): `grep -L` over `dist/assets/` must list both files |
