...
| **View-transition guard** — `startViewTransition` only from the navigation hook (_since 2026-09-25: from nowhere; `check-bundle.mjs` fails the build if any chunk contains it, see [10-touch-and-route-motion.md](10-touch-and-route-motion.md)_) | the string `startViewTransition` appears in exactly one source file (`hooks/useViewTransitionNavigate.ts`) | `grep -rl startViewTransition frontend/src --include=*.ts --include=*.tsx \| grep -v test` → one path; also an ESLint `no-restricted-properties` rule if cheap |
