...
> **Built 2026-09-19 (PR #315).** Eight numbered departures, all found by rendering the board on a `dist-local` preview and measuring it; the PR body lists each with its measurement. The ones that change what this section says are marked **[as built]** below.
...
**Interfaces:**
- `useCountUp(value: number, opts?: { duration?: number; delay?: number; from?: number }): number` — returns the displayed number; jumps immediately under reduced motion or in jsdom (no `matchMedia`). **[as built]** `from` is the starting point for the first animation only and is what makes the `delay` argument meaningful: the Display board omits it (opens on the current score and eases later changes), `EndScreen` passes `0` so the podium keeps the 0 → total roll-up it has today. Without it the hook opens on its target, `Score`'s `delay` is inert, and that roll-up silently disappears.
...
**[as built]** four corrections to the block above, each measured: the rank ring is `--ring: calc(var(--rf) * 1.9)` (a length derived from the row's type size) used by both the grid column and `.bigRank`, not a fixed `72px` / `56px` pair — a fixed ring overflows a 30 px row at the 640 px floor and undershoots the base row's own numeral, and a custom property holding an `em` re-resolves against each *using* element's font-size, so parent and child would disagree; `--row-h` / `--row-font` become `clamp(78px, 13vh, 150px)` / `clamp(1.35rem, 3.4vh, 2.75rem)` because the fixed 78 px / 1.8 rem pair was tuned for the old stacked layout and left the board using ~40 % of its column at 1080p; the reveal panel takes the sidebar's slack (`flex: 1`, content centred) so the column has no ~480 px hole between the timer and the QR, and `.side` is centred so the waiting screen's QR is not stranded in the corner; and `.revealText` wraps (three-line cap) in the sidebar.

TV grid (inside the existing `@media (min-width: 769px) and (min-height: 640px)` block, replacing `.shell`'s flex rules — **[as built]** every one of these selectors is `.shell[data-density]`, not bare `.shell`: only the live board sets `data-density`, and the ended / swept / loading branches render children with no grid area that would otherwise be auto-placed into the board's named cells. For the same reason the two short-frame refinements are re-specified at that specificity, and trim `--tv-gap` rather than `gap` so the grid keeps its column gutter):
```css
  .shell {
...
- [ ] **Step 6: EndScreen** — delete confetti (lines 10–51, 256, 265–283, and the `.confettiLayer/.confettiPiece/confetti-fall` CSS at `EndScreen.module.css:12-49`); replace the local `CountUp` with `useCountUp` inside a small `Score` component (`function Score({ value, delay }: { value: number; delay: number }) { const n = useCountUp(value, { duration: 900, delay }); return <>{n}</>; }`) so `EndScreen.test.tsx` expectations on displayed totals still hold (jsdom jumps to the final value, as the old `prefersReducedMotion()` path did). CSS: `.title { font-family: var(--font-display); color: var(--bone); background: none; text-shadow: none; }`, `.podiumCard { background: var(--surface); border: 1px solid var(--border); box-shadow: none; }`, `.gold { border-color: var(--accent); box-shadow: inset 0 3px 0 var(--accent); animation: podium-rise var(--dur-enter) var(--ease-arrive) 240ms forwards; } .gold::after { content: ""; position: absolute; inset: 0; border-radius: inherit; background: linear-gradient(115deg, transparent 40%, rgba(233,228,217,0.18) 50%, transparent 60%); transform: translateX(-100%); animation: sweep 900ms var(--ease-move) 900ms 1 forwards; pointer-events: none; } @keyframes sweep { to { transform: translateX(100%); } }` (this one gradient is the light sweep and is allowed — it is moved with transform, never repainted; **[as built]** `.gold` also needs `clip-path: inset(-32px 0 0 0)`, because `translateX(100%)` parks the band *outside* the card and `.podiumCard` cannot be `overflow: hidden` — that would clip the crown, which deliberately overhangs the top edge — so without it a bright diagonal streak sits on the black ground for the rest of the game. A negative inset expands the clip region upward for the crown while clipping the three sides the band can escape from.), `.silver { animation-delay: 120ms; } .bronze { animation-delay: 0ms; }`, delete `gold-glow`; `.medal { font-family: var(--font-display); background: var(--surface-2); border: 2px solid var(--border); color: var(--bone); }`; `.gold .medal { border-color: var(--accent); }`; `.teamName { font-family: var(--font-display); color: var(--text); } .teamScore { font-family: var(--font-display); color: var(--bone); }`; `.crown { background: var(--accent); color: var(--accent-ink); border: 3px solid var(--bg); }`; `.winnerLabel { color: var(--accent); background: transparent; font-family: var(--font-ui); }`; `.scoreboardRow` → `var(--surface)` + `var(--border)`, rank ring as on the board.

- [ ] **Step 7: Bundle guard**

**[as built]** The string match below **cannot pass**: Rolldown minifies away every literal occurrence of `auto-animate`, so `src.includes(scoped)` is false on every chunk and the final "DisplayPage chunk should contain auto-animate" check fails the build unconditionally. The shipped guard attributes modules from each chunk's **sourcemap** `sources` array instead (exact, and immune to minification); a chunk with no sourcemap is a hard failure rather than a silent pass, with the generated `rolldown-runtime-*` shim the one allowed exception. The sketch below is kept for the shape of the checks.
