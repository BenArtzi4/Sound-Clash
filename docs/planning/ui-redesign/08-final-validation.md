...
| Home `/` | **Amended by [09](09-home-colour-and-fill.md).** Large centred wordmark (mark beside the text on desktop, stacked above it under 600 px); headline "Name the song. Buzz first." in `--bone`; three role cards — Host orange, Play green, Display periwinkle — each with a 3 px left rule and matching icon, a 3-up grid on desktop and a compact icon+text row on the phone; hovering or tapping a card sweeps its colour in diagonally over 200 ms with the ink flipping to black at the wipe edge; "How to play →" a quiet text link, not a button; entrance stagger on first load only; the composition vertically centred with no dead band below | T4 + 09 |
