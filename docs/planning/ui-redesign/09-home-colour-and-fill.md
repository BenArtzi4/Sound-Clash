...
**Touch.** Phones get the same fill on `pointerdown`, retracting on release, driven by a `data-tapped` attribute rather than `:hover`. A class cannot leave a card stuck coloured after a tap — the sticky-hover failure this repo already hit on the decade pills (PR #282).
