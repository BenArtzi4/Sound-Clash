...
_Created 2026-09-17. Status (2026-09-22): **shipped and validated — Tasks 0–11 are merged and live on `https://soundclash.org`; the end-of-redesign run in [08-final-validation.md](08-final-validation.md) happened on 2026-09-22 in full mode with verdict GO ([validation/2026-09-22-final-validation.md](validation/2026-09-22-final-validation.md)); its fix PRs #329–#334 and the #339 revert are merged.** The maintainer asked for a professional, emoji-free look with smooth transitions that does not read as a generic AI-generated app, anchored on one site from the Barba.js showcase, plus a plan to prove nothing breaks. This folder is that plan; each task's as-built departures are recorded inline in [07-implementation-plan.md](07-implementation-plan.md). Every follow-up is merged (#340 status-strip gap, #341 close-out docs, #342 the three taste calls); nothing is open, and the cross-browser pass below is done. The OG image / PWA icon follow-up (decision 9) shipped 2026-09-21 in PR #327._
...
| [08-final-validation.md](08-final-validation.md) | **The end-of-redesign run**: a starter prompt for one fresh session that runs every gate, the full local e2e suite on three engines, the prod API/RPC negative-path matrix and a 30-team load check, a 13-team real-browser game on prod exercising every button and combination, the machine + looked-at-it design conformance pass, and the triage/fix/report loop with a GO / NO-GO verdict | After Task 11 merges (full mode), or after Task 8 for a drift check (partial mode). **Run 2026-09-22 (full) → GO** |
| [09-home-colour-and-fill.md](09-home-colour-and-fill.md) | The Home colour pass: warm ground, per-role hues, the hero wordmark and the diagonal hover fill | When touching Home or a colour token |
| [10-touch-and-route-motion.md](10-touch-and-route-motion.md) | **Amends 04 §4, 05 and 09 (2026-09-25):** touch gets a ripple instead of the fill, Home's intro plays only on landing, and screen changes are instant (route transitions and the wordmark morph removed), with the phone evidence behind each call | Before adding any page-change or press animation |
...
**Cross-browser pass for Task 10 — done 2026-09-22:** real Chrome and headless Chromium (transition plays, Back is instant — `ui_prod_pass` + the static audit), Firefox 151 headless against prod (transitions active, no errors, no layout shift) and, decisively, the maintainer's real iPhone Safari on a preview built without the #329 skip (Home → Host / How to play repeated, no crash — so the skip was reverted in #339). Playwright's WebKit 26.5 build still crashes on that transition; that is a build quirk of the test engine, recorded in the validation report, not a product bug.
...
| 10 | Route transitions | `useViewTransitionNavigate`, `TransitionLink`, `::view-transition-*` CSS, navigation sites | reduced-motion + Firefox fallback checks. **Removed 2026-09-25**: see [10-touch-and-route-motion.md](10-touch-and-route-motion.md) |
