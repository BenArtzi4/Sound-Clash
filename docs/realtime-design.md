...
**Final board survives the delete (I-FinalBoard).** When the rows are deleted (End game or the expiry sweep) `useGameChannel` sets `state` to `null` — but it also keeps a `finalBoard` snapshot of the last-known state so the pages can still render the final scoreboard instead of a bare "gone" line. The snapshot tracks every committed state during play; the wrinkle is that the sweep cascade-deletes `game_teams`/`game_rounds`, and those child DELETEs can arrive before the `active_games` DELETE that nulls `state`, so the states committed mid-teardown shrink team by team. A **shrinking** update is held (snapshot not overwritten) when it's part of a teardown:

- the game is `ended` — every shrink now is the post-end sweep, so the whole final board is held; **or**
- the game is expired-but-unended (an abandoned game the hourly sweep is tearing down) **and** the shrink is not a lone kick.

A **kick** — one team removed, ≥1 team still in the room, round history intact — is always honored, even in the overdue-but-unswept window (past `expires_at` but still `playing`, kept alive by the "Keep playing +1h" banner): deleting a team's row there is a real host action and the removed team must not linger on the final board. (Using the clock alone as "teardown" wrongly froze the snapshot over such a kick.) Removing the **last** team, or a shrink that also drops a round, is unmistakably the cascade → held. Residual, accepted for abandoned games only: a >4h-overrun game the host never ended, whose sweep delivers its team DELETEs before its round DELETEs, can show a partial final board (each intermediate team removal is indistinguishable from a kick without lookahead); the ended-game path and rounds-first / last-team sweeps are fully protected. All three roles read `finalBoard`: the display renders the `EndScreen` podium under the banner; the manager and team pages render the podium **and** the post-game setlist (the round history and claims are in the snapshot, and the `songs` table is durable, so the setlist resolves even after the ephemeral rows are gone). A client that never saw the live game (it opened straight onto an already-swept code, so no snapshot exists) falls back to the plain banner. See `frontend/src/hooks/useGameChannel.ts` (`finalBoard`).
