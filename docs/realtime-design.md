...
**Final board survives the delete (I-FinalBoard).** When the rows are deleted (End game or the expiry sweep) `useGameChannel` sets `state` to `null` — but it also keeps a `finalBoard` snapshot of the last-known state so the pages can still render the final scoreboard instead of a bare "gone" line. The snapshot tracks every committed state during play; the one wrinkle is that the sweep cascade-deletes `game_teams`/`game_rounds` a beat *before* `active_games`, so the states committed mid-teardown shrink team by team. Those shrinking updates are skipped **only once the game is `ended` or expired** — so a genuine kick from a live game still updates the snapshot (a kicked team must not reappear at game end), while a sweep can't erode the board before the final DELETE lands. All three roles read it: the display and team pages render the `EndScreen` podium under the banner; the manager keeps the podium **and** the post-game song export (the `songs` table is durable, so the export resolves even after the ephemeral rows are gone). A client that never saw the live game (it opened straight onto an already-swept code, so no snapshot exists) falls back to the plain banner. See `frontend/src/hooks/useGameChannel.ts` (`finalBoard`).
