...
/*
  ...
  Content-Security-Policy: default-src 'self'; script-src 'self' https://www.youtube.com https://s.ytimg.com https://static.cloudflareinsights.com 'unsafe-inline'; img-src 'self' data: https://i.ytimg.com; frame-src https://www.youtube.com https://www.youtube-nocookie.com; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://api.soundclash.org https://*.ingest.sentry.io https://*.ingest.de.sentry.io https://*.grafana.net

# content-hashed build output: cache forever, never revalidate
/assets/*
  Cache-Control: public, max-age=31536000, immutable

# un-hashed static (each of /icons/*, /og-image.jpg, /manifest.webmanifest
# is its own block): cache a day
/icons/*
...
- **Cloudflare Web Analytics:** since the zone moved to Cloudflare (2026-09-25), Cloudflare injects its analytics beacon (`<script src="https://static.cloudflareinsights.com/beacon.min.js…" integrity=…>`) into every HTML page served on `www.soundclash.org`, so the beacon's script host is in `script-src` (host-only: the injected URL carries a version path, `/beacon.min.js/v…`, which a path-specific source would not match). Its reports go to the site's own `/cdn-cgi/rum`, already covered by `connect-src 'self'` (Cloudflare's documented requirement for automatic injection). Without the `script-src` entry every page logged a CSP violation. Cloudflare states it uses no cookies or localStorage and does not fingerprint visitors; the beacon loads asynchronously and is not in the buzzer path. It is not injected on `*.pages.dev` previews. To drop it, turn off Web Analytics for the zone in the Cloudflare dashboard and remove the `script-src` entry here.
- **Caching:** content-hashed `/assets/*` are served `immutable` (a fresh hash on every change makes revalidation pointless, so the browser never re-checks them); the un-hashed static assets (`/icons/*`, `/og-image.jpg`, `/manifest.webmanifest`) get `max-age=86400`; and `index.html`, every SPA route, and `sw.js` are deliberately left on the platform default (`max-age=0, must-revalidate`) so a new deploy is always picked up and never served stale over a live game.
...
- **Search:** per-game and admin paths (`/team/*`, `/manager/game/*`, `/admin/*`, `/join/*`, `/display/*`) get `X-Robots-Tag: noindex, nofollow`, and the `sound-clash.pages.dev` alias gets `X-Robots-Tag: noindex`. See [`seo.md`](seo.md).

Backend (FastAPI) sets:
- `Strict-Transport-Security: max-age=31536000; includeSubDomains` (Render terminates TLS; HSTS still useful)
- `X-Content-Type-Options: nosniff`
- `X-Robots-Tag: noindex` (keeps the API and its `/docs` page out of search)
