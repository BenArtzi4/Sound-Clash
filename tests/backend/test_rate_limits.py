...
async def test_create_game_limit_10_per_minute(admin_client, db) -> None:
    genres = await fetch_genre_ids(db, slugs=["rock"])
    payload = {"total_rounds": 3, "selected_genres": [str(genres[0])]}

    statuses: list[int] = []
    for _ in range(11):
        resp = await admin_client.post("/games", json=payload)
...
async def test_429_envelope_shape(admin_client, db) -> None:
    genres = await fetch_genre_ids(db, slugs=["rock"])
    payload = {"total_rounds": 3, "selected_genres": [str(genres[0])]}
    last_resp = None
    for _ in range(11):
        last_resp = await admin_client.post("/games", json=payload)
